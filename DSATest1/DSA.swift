...
class DSA {
  ...
  func integerRange( _ low: Int, _ high: Int) -> Int {
    let array = Array(low...high)
    let filtered = array.filter { $0 % 5 != 0 || $0 % 10 == 0}
    print(filtered)
  ...
  func inputSum( _ arr: [Int], _ targetInt: Int) -> Bool {
    ...
    for index in 0..<arr.count {
      for nextIndex in index+1..<arr.count {
        if arr[index] + arr[nextIndex] == targetInt {
          tempBool = true
        }
      }
    }
  ...
  func recursiveSum( _ input: [Int]) -> Int {
    ...
    return input[0] + recursiveSum(Array(input[1...]))
  }
